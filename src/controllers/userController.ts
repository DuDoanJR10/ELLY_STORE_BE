...
class userController {
  async register(req: Request, res: Response) {
    ...
    try {
      ...
      await redisService.setValue(
        `${KeyRedis.VERIFY_EMAIL}_${newUser.id}`,
        code.toString(),
        5 * 60
      );
  ...
  async verifyEmail(req: Request, res: Response) {
    ...
    try {
      ...
      const codeInRedis = await redisService.getValue(
        `${KeyRedis.VERIFY_EMAIL}_${user.id}`
      );
  ...
  async getMe(req: CustomRequest, res: Response) {
    try {
      ...
      const user = await UserModel.findOne({ where: { id: userToken.userId } });

      if (!user) {
        return responseInterceptor(res, 400, "Tài khoản không tồn tại", null);
      }

      return responseInterceptor(res, 200, "Thành công", {
        id: user.id,
        name: user.name,
        email: user.email,
        role: user.role,
